...
#def lagrangian_point_2d(path, number, step, rank=4):
    #time_coord = time(path, number, step)
    #coord = {'time': time_coord}
    
    #xx, yy, zz = [], [], []
    #x, y, z, cc, cc0 =[], [], [], [], []
    #for cont in range(0, number + step, step):
        #for rank in range(rank):
            #filename ='step_{}-rank{}.txt'.format(cont,rank)
        ...
        #xx = np.append(xx, x)
        #yy = np.append(yy, y)
        #print(time_coord.shape, xx.shape)
        #cc0 = cc0.astype("int")
    
        #data_vars = {'x_coordinate':(['time'], x[cc0%10==0]/1000),
                     #'y_coordinate':(['time'], y[cc0%10==0]/1000)}
        #dataset = xr.Dataset(data_vars, coords=coord)
