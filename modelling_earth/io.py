...
def _read_viscosity(path):
    """
    Read the viscosity files generated by MD3D
    ...
    -------
    viscosity : np.array
        Array containing the MD3D output viscosity data.
    ...
    vis, viscosity = [], []
    # Get viscosity files
    viscosity_files = [i for i in os.listdir(path) if "step_" in i]
    # Read the data
    for i, (step, time) in enumerate(zip(steps, times)):
        # Determine the rank value on the first step and check it for following steps
        step_files = [i for i in viscosity_files if "step_{}-".format(step) in i]
        if i == 0:
            n_rank = len(step_files)
        else:
            if len(step_files) != n_rank:
                raise ValueError(
                    "Invalid number of ranks '{}' for step'{}'".format(len(step_files), 
                                                                            step))
        # Read viscosity for the current step
        visc = _read_md3d_single_viscosity(path, step, time, n_rank)
        viscosity.append(visc)
