...
def linear_velocity(coordinates, z_start, velocity_bottom):
    """
    ...
    velocity_bottom = velocity_bottom / (365 * 24 * 3600)
    ...
    velocity["velocity_x"], velocity["velocity_z"] = np.zeros(shape), np.zeros(shape)
    # Calculate the velocity assume a linear increase with depth
    lin_velocity_x = velocity_bottom[0] * (velocity.z - z_start) / velocity.z[-1]
    lin_velocity_z = velocity_bottom[1] * (velocity.z - z_start) / velocity.z[-1]
    ...
    velocity.x.where(~condition, lin_velocity_x)
    velocity.z.where(~condition, lin_velocity_z)
